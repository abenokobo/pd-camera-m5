...
///
static void IRAM_ATTR usb_ctrl_cb(usb_transfer_t *transfer)
{
    ModUSBacmDevice *dev = (ModUSBacmDevice *)transfer->context;
...
///
static void IRAM_ATTR usb_read_cb(usb_transfer_t *transfer)
{
    ModUSBacmDevice *dev = (ModUSBacmDevice *)transfer->context;
...
///
static void IRAM_ATTR usb_write_cb(usb_transfer_t *transfer)
{
    ModUSBacmDevice *dev = (ModUSBacmDevice *)transfer->context;
...
///
ModUSBacmDevice::ModUSBacmDevice(const usb_config_desc_t* config_desc, const usb_intf_desc_t *intf, size_t n, USBhost* host)
{
    _host = host;
...
///
ModUSBacmDevice::~ModUSBacmDevice()
{
}


///
bool ModUSBacmDevice::init()
{
    usb_device_info_t info = _host->getDeviceInfo();

    esp_err_t err = usb_host_transfer_alloc(TRANSFER_ALLOC_SIZE_WRITE, 0, &xfer_write);
    ...
    err = usb_host_transfer_alloc(TRANSFER_ALLOC_SIZE_READ, 0, &xfer_read);
...
///
void ModUSBacmDevice::setControlLine(bool dtr, bool rts)
{
...
///
void ModUSBacmDevice::setLineCoding(uint32_t bitrate, uint8_t cf, uint8_t parity, uint8_t bits)
{
    line_coding_t data;
...
///
void ModUSBacmDevice::getLineCoding()
{
...
void ModUSBacmDevice::INDATA()
{
    ...
    xfer_read->num_bytes = TRANSFER_ALLOC_SIZE_READ;
    esp_err_t err = usb_host_transfer_submit(xfer_read);
}


///
SemaphoreHandle_t ModUSBacmDevice::OutDataAsync(uint8_t *data, size_t len)
{
    if ((!connected)
     || (!len)
     || (!data))
    ...
    m_spOutData = std::make_shared<ModUSBacmDeviceOutData>();
    if (m_spOutData == NULL)
    {
        return NULL;
    }

    auto ret = m_spOutData->Initialize(xfer_write, data, len);
    if (ret == NULL)
    {
        m_spOutData = NULL;
    }
...
///
void ModUSBacmDevice::OnOutDataCB()
{
    if (m_spOutData)
    {
        m_spOutData->OnOutDataCB();
    }
}


///
void ModUSBacmDevice::DoOutData()
{
    if (m_spOutData)
    {
        if (m_spOutData->DoOutData())
        {
            // finished
            m_spOutData = NULL;
        }
...
///
bool ModUSBacmDevice::isConnected()
{
    return connected;
}
...
///
void ModUSBacmDevice::onEvent(cdc_event_cb_t _cb)
{
    event_cb = _cb;
}
...
///
void ModUSBacmDevice::_callback(int event, usb_transfer_t *transfer)
{
    switch (event)
    {
    case CDC_DATA_IN:
